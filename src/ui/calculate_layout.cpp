...
static void set_border_percentages(Container *cont) {
	...
	if (cont->border_bottom >= 0) {
		float border_bottom =
			mod_clamp(height / 100 * cont->border_bottom, cont->border_bottom_max, cont->border_bottom_min);
	...
	if (cont->border_right >= 0) {
		float border_right =
			mod_clamp(width / 100 * cont->border_right, cont->border_right_max, cont->border_right_min);
...
static void set_button_widths(Button *button) {
	...
	if (button->border_left >= 0) {
		border_left =
			mod_clamp(text_width / 100 * button->border_left, button->border_left_max, button->border_left_min);
	...
	if (button->border_right >= 0) {
		border_right =
			mod_clamp(text_width / 100 * button->border_right, button->border_right_max, button->border_right_min);
